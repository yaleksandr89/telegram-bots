...
class EchoTelegramBot
{
     ...
     * @return void
     */
    public function addToLogs(array $incomingMsg): void
    {
        if (count($incomingMsg) > 0) {
            ob_start();
            echo '------------------' . PHP_EOL;
            if (array_key_exists('message', $incomingMsg)) {
                echo '===[' . date('d-m-Y H:i:s', $incomingMsg['message']['date']) . ']===' . PHP_EOL;
            }

            if (array_key_exists('edited_message', $incomingMsg)) {
                echo '===[' . date('d-m-Y H:i:s', $incomingMsg['edited_message']['edit_date']) . ']===' . PHP_EOL;
            }
